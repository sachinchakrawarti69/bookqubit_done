...
const LoginPage = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        // Redirect after 2 seconds or let user choose
        setTimeout(() => {
          router.push("/");
        }, 2000);
      }
  ...
  if (isAlreadyLoggedIn) {
    return (
      <div className={`${theme.background?.section || (isDarkMode ? 'bg-gray-900' : 'bg-gray-50')} min-h-screen flex items-center justify-center py-12 px-4`}>
        <div className="max-w-md w-full text-center">
          <div className={`${theme.background?.section || (isDarkMode ? 'bg-gray-800' : 'bg-white')} rounded-xl p-8 shadow-xl`}>
            <div className="w-16 h-16 mx-auto mb-4 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center">
              <svg className="w-8 h-8 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h2 className={`text-2xl font-bold mb-2 ${theme.textColors?.primary || 'text-gray-900'}`}>Already Logged In</h2>
            <p className={`mb-6 ${theme.textColors?.secondary || 'text-gray-600'}`}>You are already signed in. Redirecting to home page...</p>
            <Link href="/" className={`inline-block px-6 py-2 ${theme.buttonColors?.primaryButton?.background || 'bg-sky-600'} text-white rounded-lg`}>
              Go to Home
            </Link>
          </div>
        </div>
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <div className={`p-3 rounded-full ${theme.background?.navigationDots || (isDarkMode ? 'bg-gray-800' : 'bg-gray-100')}`}>
              <FaEnvelope className={`text-3xl ${theme.textColors?.highlight || 'text-sky-600'}`} />
            </div>
          </div>
        ...
        <div className={`${theme.background?.section || (isDarkMode ? 'bg-gray-800' : 'bg-white')} ${theme.border?.default || 'border border-gray-200 dark:border-gray-700'} rounded-xl p-6 md:p-8 shadow-xl`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className={`text-sm ${theme.textColors?.secondary || 'text-gray-600'}`}>Remember me</span>
              </label>
              <Link
          ...
          <p className={`mt-6 text-center text-sm ${theme.textColors?.secondary || 'text-gray-600'}`}>
            ...
            <Link href="/auth/register" className={`${theme.textColors?.highlight || 'text-sky-600'} hover:underline font-medium`}>
              Sign up
            </Link>
          </p>
        </div>
