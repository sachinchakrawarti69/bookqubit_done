...
export default function ComicsPage() {
  ...
  const publishers = ["All", ...new Set(ComicsData.map(comic => comic.publisher).filter(Boolean))];
  const categories = ["All", ...new Set(ComicsData.map(comic => comic.category).filter(Boolean))];
  ...
  const filteredComics = comics.filter(comic => {
    ...
    const matchesPublisher = selectedPublisher === "All" || comic.publisher === selectedPublisher;
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <svg key={`star-${i}`} className={`w-4 h-4 ${theme.iconColors?.starFilled || 'text-amber-400'}`} fill="currentColor" viewBox="0 0 20 20">
    ...
    if (hasHalfStar) {
      stars.push(
        <svg key="half-star" className={`w-4 h-4 ${theme.iconColors?.starFilled || 'text-amber-400'}`} fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      );
    }
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <svg key={`empty-${i}`} className={`w-4 h-4 ${theme.iconColors?.starEmpty || 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      );
    }
    ...
    <main className={`min-h-screen ${theme.background?.section || ''}`}>
      ...
      <section className={`${theme.layout?.sectionPadding || 'py-12 px-4 sm:px-6 lg:px-8'} text-center ${theme.background?.navigationDots || ''}`}>
        <div className={`${theme.layout?.containerWidth || 'max-w-7xl'} mx-auto`}>
          ...
          <p className={`text-lg md:text-xl mb-8 ${theme.textColors?.secondary || ''} max-w-2xl mx-auto`}>
            Discover legendary comics from Marvel, DC, Raj Comics, and more. From Golden Age classics to modern masterpieces!
          </p>
        </div>
      </section>
                  ...
                  >
                    <Link href={`/comics/${comic.id}`}>
                      <div className="p-4">
                        ...
                        <p className={`text-sm mb-1 ${theme.textColors?.secondary || ''}`}>
                          {comic.publisher}
                        </p>
                        <p className={`text-xs mb-2 ${theme.textColors?.highlight || ''}`}>
                        ...
                        {comic.charactersIntroduced && comic.charactersIntroduced.length > 0 && (
                          <div className="mb-3">
                            <p className={`text-xs ${theme.textColors?.secondary || ''}`}>
                              <span className="font-semibold">Introduced:</span> {comic.charactersIntroduced.slice(0, 2).join(", ")}
                              {comic.charactersIntroduced.length > 2 && "..."}
                            </p>
                          </div>
                        )}
                        <div className="flex justify-between items-center">
                          <span className={`text-sm font-bold ${theme.textColors?.highlight || ''}`}>
                            {comic.coverPrice}
                          </span>
      ...
      <section className={`py-16 px-4 sm:px-6 lg:px-8 ${theme.background?.navigationDots || ''}`}>
        <div className={`${theme.layout?.containerWidth || 'max-w-7xl'} mx-auto text-center`}>
          <h2 className={`text-3xl font-bold mb-8 ${theme.textColors?.primary || ''}`}>
            Featured Publishers
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {['Marvel Comics', 'Raj Comics', 'Timely Comics', 'Apex Comics'].map((publisher) => (
              <button
                key={publisher}
                onClick={() => {
