...
const BookCompactCard = ({ book, collections = [], showCollections = true, className = "" }) => {
  ...
  const bookCollections = collections.length > 0 ? collections : getCollectionsAsArray(book.collection);
      ...
      <div className="p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1">
            {/* Title */}
            <h3 className={`text-lg font-bold ${theme.textColors?.primary || (isDarkMode ? 'text-white' : 'text-gray-900')} mb-1 line-clamp-2`}>
              {book.title}
            </h3>
            ...
            <div className="flex items-center mb-3">
              ...
              <span className={`text-xs ml-1 ${theme.textColors?.secondary || (isDarkMode ? 'text-gray-400' : 'text-gray-600')}`}>
                ({book.rating?.toFixed(1) || "0"})
              </span>
            </div>

            {/* Description */}
            <p className={`text-sm ${theme.textColors?.secondary || (isDarkMode ? 'text-gray-400' : 'text-gray-600')} line-clamp-2 mb-3`}>
              {book.description}
            </p>
